...
import numpy as np
import pdb
from .adapter import Conv2dAdapterBottleneck as AdapterBottleneck

__all__ = ['SENet', 'senet154', 'se_resnet50', 'se_resnet101', 'se_resnet152',
           'se_resnext50_32x4d', 'se_resnext101_32x4d']

model_path = 'checkpoints/se_resnet50-ce0d4300.pth'
...
class CSEResNetBottleneck(Bottleneck):
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
                 downsample=None, add_adapter=False):
        super(CSEResNetBottleneck, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False,
                               stride=stride)
        self.bn1 = nn.BatchNorm2d(planes)
        ...
        if add_adapter :
            self.adapter = AdapterBottleneck(planes * 4)
        self.stride = stride
...
class CSENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 ...
                 downsample_padding=1, num_classes=1000, ems=False, add_adapter=False):
        """
        Parameters
        ----------
        ...
        layer0_modules.append(('pool', nn.MaxPool2d(3, stride=2, padding=0, dilation=1, 
        ...
        if self.ems:
            ...
            # self.last_linear = NormLinear(512 * block.expansion, num_classes)

    def _make_layer(self, block, planes, blocks, groups, reduction, stride=1,
                    downsample_kernel_size=1, downsample_padding=0, add_adapter=False):
        downsample = None
        ...
        layers.append(block(self.inplanes, planes, groups, reduction, stride,
                            downsample, add_adapter))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups, reduction, add_adapter=add_adapter))
...
class CSENet_hashing(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 inplanes=128, input_3x3=True, downsample_kernel_size=3,
                 downsample_padding=1, num_classes=1000, ems=False, hashing_dim = 64, add_adapter=False):
        """
        Parameters
        ----------
        block (nn.Module): Bottleneck class.
        ...
        # # modify adapter position and numbers
        # add_adapter = False
        self.layer3 = self._make_layer(
            block,
            planes=256,
        ...
        self.dropout = nn.Dropout(dropout_p) if dropout_p is not None else None
        self.ems = ems
        self.second_last_linear = nn.Linear(512 * block.expansion, hashing_dim)  # 2048 --> hashing_dim
        # if self.ems:
    ...
    def hashing(self, x):
        ...
        if self.dropout is not None:
            x = self.dropout(x)
        x = x.view(x.size(0), -1)
        x = self.second_last_linear(x)  #  [N, C]--> [N, hashingdim]
        return x
...
def initialize_pretrained_model_hashing(model, hashing_dim, num_classes, settings):
    ...
    for kk,vv in pretrained_state_dict.items():
        ...
        if 'se_module.fc2.weight' in kk:
            pretrained_state_dict[kk] = torch.cat([torch.squeeze(vv), torch.zeros(vv.size()[0], 1)], dim=1)
    #pdb.set_trace()
    old_weight = pretrained_state_dict['last_linear.weight']  # 取出最后一层分类器的权重，后面计算svd
    print(old_weight.shape)  # (1000, 2048)
    u, s, vh = np.linalg.svd(old_weight, full_matrices=True)
#    u_new = u[:hashing_dim,:hashing_dim]
    s_new = np.diag(s)[:hashing_dim,:hashing_dim]  # 对角线元素
...
def cse_resnet50(num_classes=1000, pretrained='imagenet', ems=False, add_adapter=False):
    model = CSENet(CSEResNetBottleneck, [3, 4, 6, 3], groups=1, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  num_classes=num_classes, ems=ems, add_adapter=add_adapter)
...
def cse_resnet50_hashing(hashing_dim, num_classes=1000, pretrained='imagenet', ems=False, add_adapter=True):
    model = CSENet_hashing(CSEResNetBottleneck, [3, 4, 6, 3], groups=1, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  num_classes=num_classes, ems=ems, hashing_dim=hashing_dim, add_adapter=add_adapter)
