...
def quickdraw_train_load(args, transformations=None):
    """返回测试集和训练集"""
    sketchy_train = QuickDrawDataset(split='train', version='sketch', zero_version=args.zero_version, \
                                     transform=transformations['sk'], aug=True)
    photo_train = QuickDrawDataset(split='train',  version='photo', zero_version=args.zero_version, \
                                    transform=transformations['im'], aug=True)                 
    sketchy_test = QuickDrawDataset(split='val',  version='sketch', zero_version=args.zero_version, \
                                    transform=transformations['sk'], aug=False)
    photo_test = QuickDrawDataset(split='val',  version='photo',zero_version=args.zero_version,\
                                    transform=transformations['im'], aug=False)
    prompt = args.prompt
    if prompt in ['coop16', 'PROM']:
        prompt = 'naive'
    class_embedding_file = f'data/QuickDraw/{args.zero_version}/class_embs_{args.arch_CLIP}_{prompt}.pickle'
    class_labels_file = f'{sketchy_train.root_dir}/{args.zero_version}/cname_cid.txt'
    with open(class_labels_file) as fp:
        all_class = [c.split()[0] for c in fp.readlines()] # read the train category labels

    with open(class_embedding_file, 'rb') as fh:
        class_embs = pickle.load(fh)
    return sketchy_train, photo_train, sketchy_test, photo_test, class_embs, all_class

def quickdraw_zero_load(args, transformations=None):
    sketchy_zero = QuickDrawDataset(split='zero',  version='sketch', \
                                transform=transformations['sk'], aug=False)
    photo_zero = QuickDrawDataset(split='zero',  version='photo', \
                                    transform=transformations['im'], aug=False)
...
def tuberlin_train_load(args, cid_mask=False, transformations=None):
    """返回测试集和训练集"""
    sketchy_train = TUBerlinDataset(split='train',  zero_version = args.zero_version, cid_mask=cid_mask, \
                                     transform=transformations['sk'], aug=True)
    photo_train = TUBerlinDataset(split='train',  version='ImageResized_ready', zero_version = args.zero_version,\
                                  cid_mask=cid_mask, transform=transformations['im'], aug=True)
    sketchy_test = TUBerlinDataset(split='zero',  zero_version = args.zero_version, cid_mask=cid_mask, \
                                    transform=transformations['sk'], aug=False)
    photo_test =   TUBerlinDataset(split='zero',  version='ImageResized_ready', zero_version = args.zero_version,\
                                    cid_mask=cid_mask, transform=transformations['im'], aug=False)                      
    prompt = args.prompt
    if prompt == 'coop16':
        prompt = 'naive'
    class_embedding_file = f'data/TUBerlin/{args.zero_version}/class_embs_{args.arch_CLIP}_{prompt}.pickle'
    class_labels_file = f'{sketchy_train.root_dir}/{args.zero_version}/cname_cid.txt'
    with open(class_labels_file) as fp:
        all_class = [c.split()[0] for c in fp.readlines()]
...
def tuberlin_zero_load(args, transformations=None):
    sketchy_zero = TUBerlinDataset(split='zero',  zero_version = args.zero_version, \
                                    transform=transformations['sk'], aug=False)
    image_zero = TUBerlinDataset(split='zero',  version='ImageResized_ready', zero_version = args.zero_version,\
                                    transform=transformations['im'], aug=False)
...
class TUBerlinDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cid_mask:
            mask = self.cid_matrix[label]  # Returns each row of the similarity matrix [batch, 1000]
            return img, label, mask
        return img, label
        # return img
...
def sketchy_train_load(args, cid_mask=False, transformations=None):
    """返回测试集和训练集"""
    sketchy_train = SketchyDataset(split='train',  zero_version = args.zero_version, cid_mask=cid_mask, \
                                     transform=transformations['sk'], aug=True)
    photo_train = SketchyDataset(split='train',  version='all_photo', zero_version = args.zero_version, \
                                    cid_mask=cid_mask,transform=transformations['im'], aug=True)                 
    sketchy_test = SketchyDataset(split='zero',  zero_version = args.zero_version, cid_mask=cid_mask, \
                                    transform=transformations['sk'], aug=False)
    photo_test = SketchyDataset(split='zero',  version='all_photo', zero_version = args.zero_version,\
                                     cid_mask=cid_mask, transform=transformations['im'], aug=False)
    prompt = args.prompt
    if prompt in ['coop16', 'PROM']:
        prompt = 'naive'
    # class_embedding_file = f'data/Sketchy/{args.zero_version}/class_embs_siglip-base-p16_naive.pickle'
    class_embedding_file = f'data/Sketchy/{args.zero_version}/class_embs_{args.arch_CLIP}_{prompt}.pickle'
    class_labels_file = f'{sketchy_train.root_dir}/{args.zero_version}/cname_cid.txt'
    with open(class_labels_file) as fp:
        all_class = [c.split()[0] for c in fp.readlines()] # Read the train category labels
...
def sketchy_zero_load(args, transformations=None):
    sketchy_zero = SketchyDataset(split='zero',  zero_version = args.zero_version, \
                                transform=transformations['sk'], aug=False)
    photo_zero = SketchyDataset(split='zero',  version='all_photo', zero_version = args.zero_version,\
                                    transform=transformations['im'], aug=False)
    return sketchy_zero, photo_zero
...
def load_dataset(args, transformations):
    if args.dataset in ['sketchy_split1', 'sketchy_split2']:
        sketchy_train, photo_train, sketchy_test, photo_test, class_smbs, class_name = sketchy_train_load(args, transformations=transformations)
        ...
        # sk_test_loader = DataLoader(dataset=sketchy_test, batch_size=args.batch_size * 4, shuffle=False, num_workers=4)
        # im_test_loader = DataLoader(dataset=photo_test, batch_size=args.batch_size * 4, shuffle=False, num_workers=4)
        sk_test_loader = DataLoader(dataset=sketchy_test, batch_size=args.batch_size, shuffle=True, num_workers=4)
        im_test_loader = DataLoader(dataset=photo_test, batch_size=args.batch_size, shuffle=True, num_workers=4)
    elif args.dataset == 'tuberlin':
        sketchy_train, photo_train, sketchy_test, photo_test, class_smbs, class_name = tuberlin_train_load(args, transformations=transformations)
        sketch_train_loader = DataLoader(dataset=sketchy_train, batch_size=args.batch_size // 5, shuffle=True, num_workers=4)
        photo_train_loader = DataLoader(dataset=photo_train,batch_size=args.batch_size // 5 * 9, shuffle=True, num_workers=4)
        ...
        sketchy_train, photo_train, sketchy_test, photo_test, class_smbs, class_name = quickdraw_train_load(args, transformations=transformations)
        sketch_train_loader = DataLoader(dataset=sketchy_train, batch_size=args.batch_size, shuffle=True, num_workers=4)
        photo_train_loader = DataLoader(dataset=photo_train,batch_size=args.batch_size, shuffle=True, num_workers=4)
        sk_test_loader = DataLoader(dataset=sketchy_test, batch_size=args.batch_size, shuffle=False, num_workers=4)
        im_test_loader = DataLoader(dataset=photo_test, batch_size=args.batch_size, shuffle=False, num_workers=4)
    ...
    return sketch_train_loader, photo_train_loader, sk_test_loader, im_test_loader, class_smbs, class_name
...
def create_dict_texts(texts):
    """Create a dictionary of key:number value:class_name"""
